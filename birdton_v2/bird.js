...
class Birdton {
    constructor() {
        ...
        this.headers = {
            ...
            'User-Agent': 'Mozilla/5.0 (Linux; Android 11; SAMSUNG SM-A546B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.105 Mobile Safari/537.36'
        };
    ...
    async main() {
        ...
        while (true) {
            for (let [index, token] of tokens.entries()) {
                let tgData = "";
                if (token.startsWith('query_id')) {
                    tgData = token.split('&')[1].split('=')[1];
                } else {
                    tgData = token.split('&')[0].split('=')[1];
                }
            ...
            const randomTime = Math.floor(Math.random() * (3600 - 900 + 1)) + 900;
            this.log(`Chờ ${Math.floor(randomTime / 60)} phút trước khi chạy lại tất cả các tài khoản...`);
