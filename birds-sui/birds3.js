    ...
    async checkAndJoinGuild(telegramauth) {
        ...
        try {
            ...
            if (guildStatusResponse.data === null) {
                try {
                    const joinResponse = await axios.get("https://api.birds.dog/guild/join/671b0b55098679d7ffb97a70", { headers });
                    
                    if (joinResponse.data === "ok") {
                        this.log("Gia nhập thành công guild Dân Cày Airdrop", 'success');
                    }
                } catch (joinError) {
                ...
                if (guildData.guild._id === "671b0b55098679d7ffb97a70") {
                    this.log("Bạn đã gia nhập guild Dân Cày Airdrop", 'info');
                } else {
