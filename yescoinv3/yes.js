...
class YesCoinBot {
    ...
    async log(msg, type = 'info') {
        ...
        switch (type) {
            case 'success':
                console.log(`[${timestamp}] [YesCoin][*] ${msg}`.green);
                break;
            case 'error':
                console.log(`[${timestamp}] [YesCoin][!] ${msg}`.red);
                break;
            case 'warning':
                console.log(`[${timestamp}] [YesCoin][*] ${msg}`.yellow);
                break;
            default:
                console.log(`[${timestamp}] [YesCoin][*] ${msg}`.blue);
        }
    ...
    async handleSwipeBot(token) {
        ...
        try {
            const accountBuildInfo = await axios.get(url, { headers: this.headers(token) });
            if (accountBuildInfo.data.code === 0) {
                const { swipeBotLevel, openSwipeBot } = accountBuildInfo.data.data;
                if (swipeBotLevel < 1) {
                ...
                if (swipeBotLevel >= 1 && openSwipeBot) {
                    ...
                    const offlineBonusInfoRes = await axios.get(offlineBonusUrl, { headers: this.headers(token) });
                    const offlineBonusInfo = offlineBonusInfoRes.data;
                    if (offlineBonusInfo.code === 0 && offlineBonusInfo.data.length > 0) {
                        ...
                        const claimResponse = await axios.post(claimUrl, claimData, { headers: headers });
                        if (claimResponse.data.code === 0) {
                            ...
                            await this.log(`Claim offline bonus thất bại: ${claimResponse.data.message}`, 'error');
                        }
                    }
                }
            } else {
                await this.log('Không thể lấy thông tin SwipeBot', 'error');
                this.log(accountBuildInfo.code);
            }
        } catch (error) {
    ...
    async performDailySignIn(token) {
        try {
            const secretKey = '6863b339db454f5bbd42ffb5b5ac9841';
            ...
            const signInListResponse = await axios.get(signInListUrl, { headers: this.headers(token) });
            ...
            if (signInListResponse.code === 0) {
                ...
                if (availableSignIn) {
                    ...
                    const signInResponse = await axios.post(signInUrl, signInData, { headers: headers });
    ...
    async performDailyMissions(token) {
        try {
            const dailyMissionsUrl = 'https://api-backend.yescoin.gold/mission/getDailyMission';
            const dailyMissionsResponse = await axios.get(dailyMissionsUrl, { headers: this.headers(token) });
            ...
            if (dailyMissionsResponse.code === 0) {
                for (const mission of dailyMissionsResponse.data) {
                    if (mission.missionStatus === 0) {
                        ...
                        await axios.post(clickUrl, mission.missionId, { headers: this.headers(token) });

                        const checkUrl = 'https://api-backend.yescoin.gold/mission/checkDailyMission';
                        const checkResponse = await axios.post(clickUrl, mission.missionId, { headers: this.headers(token) });

                        if (checkResponse.code === 0 && checkResponse.data === true) {
                            const claimUrl = 'https://api-backend.yescoin.gold/mission/claimReward';
                            const claimResponse = await axios.post(claimUrl, mission.missionId, { headers: this.headers(token) });
        ...
        while (true) {
            for (let i = 0; i < this.accounts.length; i++) {
                ...
                if (squadInfo && squadInfo.data.isJoinSquad) {
                    ...
                    await this.log('Squad: Bạn không ở trong Squad, gia nhập Squad để thêm điểm.', 'warning');
                    await this.randomDelay();
                    const joinResult = await this.joinSquad(token, "t.me/binance_announcements");
                    if (joinResult) {
